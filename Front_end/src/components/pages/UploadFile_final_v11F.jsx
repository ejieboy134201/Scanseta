...
const UploadFile = ({ goNext, goBack, goToMedInfo, setMedicineData }) => {
  ...
  const handleCaptureImage = async () => {
    if (webcamRef.current) {
      ...
      const fileBlob = dataURItoBlob(imageSrc); // Convert data URI to Blob
      fileBlob.name = "captured_image.jpg"; // Add a name for consistency
      setSelectedFile(fileBlob); // Correctly set the selectedFile state
      ...
      setUploadStatus('Image captured successfully. Ready to scan.');
      setStatusMessage('Captured Successfully. Click Scan to process.');
      setTimeout(() => setStatusMessage(''), 5000);
    } else {
