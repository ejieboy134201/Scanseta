...
const UploadFile = ({ goNext, goBack, goToMedInfo, setMedicineData }) => {
  ...
  const handleFileChange = (e) => {
    ...
    if (file) {
      ...
      setTimeout(() => setStatusMessage(''), 3000); // Reset after 3 seconds
    }
  };
  ...
  const handleCaptureImage = async () => {
    if (webcamRef.current) {
      ...
      setSelectedFile(dataURItoBlob(imageSrc));
    }
  
    if (!selectedFile) {
      setStatusMessage('No image selected. Please capture/upload an image first.');
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Set the image with bounding box as the preview image
        if (result.image_with_bbox) {
          setPreviewImage(`data:image/jpeg;base64,${result.image_with_bbox}`);
      }
  ...
  const handleScanImage = async () => {
    ...
    const formData = new FormData();
    formData.append('file', selectedFile);
    setProgress(40); // Midway progress
    ...
    try {
      ...
      const result = await response.json();
      console.log(result); // Inspect the entire result

      if (result.image) {
        const imageBase64 = `data:image/png;base64,${result.image}`; // Assuming image is PNG, adjust if it's JPEG
        setPreviewImage(imageBase64); // Set the previewImage to the base64 string
        setProgress(70);
      }
      ...
      if (response.ok) {
        ...
        setProgress(100); // Completion progress
        console.log(result.image_with_bbox); // Check if this value is correct
        console.log(result); // Ensure that the backend response contains 'image_with_bbox' as expected
